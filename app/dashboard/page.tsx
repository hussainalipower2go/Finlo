...
function SettingsPage({ colors, displayName, userEmail, onSignOut, currency, onCurrencyChange, supabase, lang = "en", onLanguageChange, plan = "professional", onPlanChange }: { colors: Colors; displayName: string; userEmail: string; onSignOut: () => void; currency: string; onCurrencyChange: (c: string) => void; supabase: ReturnType<typeof createClient>; lang?: string; onLanguageChange: (l: LangCode) => void; plan?: "beginner" | "professional"; onPlanChange: (p: "beginner" | "professional") => void }) {
  ...
  const exportMonthLabel = (m: string) =>
    new Date(m + "-01").toLocaleDateString("en-PK", { month: "short", year: "2-digit" });
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const summary = {
        ...
        totalExpense: exp.reduce((s, e) => s + Number(e.amount), 0),
    ...
    <div style={{ maxWidth: 680, display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div style={{ padding: "22px 24px", borderRadius: 16, background: colors.card, border: `1px solid ${colors.cardBorder}` }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => onPlanChange("beginner")} style={{ flex: 1, padding: "14px 16px", borderRadius: 12, border: `1px solid ${plan === "beginner" ? "#0A193D" : colors.cardBorder}`, background: plan === "beginner" ? "rgba(10,25,61,0.08)" : "transparent", cursor: "pointer", textAlign: "left", fontFamily: "inherit" }}>
          ...
          <button onClick={() => onPlanChange("professional")} style={{ flex: 1, padding: "14px 16px", borderRadius: 12, border: `1px solid ${plan === "professional" ? "#0A193D" : colors.cardBorder}`, background: plan === "professional" ? "rgba(10,25,61,0.08)" : "transparent", cursor: "pointer", textAlign: "left", fontFamily: "inherit" }}>
